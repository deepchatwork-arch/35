...
import { SIGNAL_COMPONENT_KEYS, type IndicatorConfig, type SignalComponentKey } from '@/types/domain';
...
export function SettingsPanel({ onClose }: { onClose: () => void }) {
  ...
  const demoStagePercents = useDemoAccountStore((s) => s.stagePercents);
  const setStagePercentAction = useDemoAccountStore((s) => s.setStagePercent);
          ...
          <Section title="Демо-счёт">
            <div className="flex flex-col gap-3 rounded-lg border border-base-800 bg-base-900 p-3">
              ...
              <p className="text-2xs text-base-500">
                Стадия 0 — фиксированная сумма сделки в $. Стадии 1–3 — проценты от суммы Стадии 0. Мартингейл ведётся отдельно по каждому инструменту (символ + таймфрейм).
              </p>
              <NumberInput
                label="Баланс ($)"
                value={demoBalance}
              ...
              <NumberInput
                label="Стадия 1 (%)"
                value={demoStagePercents[0]}
                min={0}
                max={10000}
                step={0.1}
                onChange={(v) => setStagePercentAction(1, v)}
              />
              <NumberInput
                label="Стадия 2 (%)"
                value={demoStagePercents[1]}
                min={0}
                max={10000}
                step={0.1}
                onChange={(v) => setStagePercentAction(2, v)}
              />
              <NumberInput
                label="Стадия 3 (%)"
                value={demoStagePercents[2]}
                min={0}
                max={10000}
                step={0.1}
                onChange={(v) => setStagePercentAction(3, v)}
              />
              <NumberInput
