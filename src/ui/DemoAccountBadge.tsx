...
export function DemoAccountBadge() {
  ...
  const stagePercents = useDemoAccountStore((s) => s.stagePercents);
  ...
  const activeStake = getStageStake(activeStage, stage0Amount, stagePercents);

  const parallelSeries = Object.entries(martingale)
    .filter(([key, ms]) => key !== activeKey && ms.stage > 0 && !ms.halted)
    .map(([key, ms]) => {
      const [sym, tf] = key.split(':');
      return { key, symbolId: sym, timeframe: tf, stage: ms.stage, stake: getStageStake(ms.stage, stage0Amount, stagePercents) };
    });
          ...
          <div className="relative flex max-h-[85dvh] w-full max-w-sm flex-col rounded-2xl border border-base-800 bg-base-950 animate-slide-up">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4">
              ...
              <p className="mb-4 text-2xs text-base-500">
                Стадия 0 — фиксированная сумма в $. Стадии 1–3 — проценты от суммы Стадии 0. Мартингейл ведётся отдельно по каждому инструменту (символ + таймфрейм).
              </p>
