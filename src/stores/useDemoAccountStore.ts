...
export interface DemoTrade {
  ...
  candleTime: number;
  stage: Stage;
  stakeConfigAtOpen: { stage0Amount: number; stagePercents: [number, number, number] };
}
...
interface DemoAccountState {
  ...
  setStagePercent: (stage: 1 | 2 | 3, percent: number) => void;
...
const DEFAULT_STAGE_PERCENTS: [number, number, number] = [250, 500, 1000];
...
export function getStageStake(
  ...
  return round2(stage0Amount * stagePercents[stage - 1] / 100);
}
...
export const useDemoAccountStore = create<DemoAccountState>()(
  persist(
    (set, get) => ({
      ...
      stagePercents: DEFAULT_STAGE_PERCENTS,
      ...
      openTrade: (signal, knownOpenPrice) => {
        ...
        const desiredStake = getStageStake(currentStage, state.stage0Amount, state.stagePercents);
        ...
        const trade: DemoTrade = {
          ...
          stakeConfigAtOpen: {
            ...
            stagePercents: state.stagePercents,
          },
        };
        set({
      ...
      setStagePercent: (stage, percent) =>
        set((s) => {
          const newPercents = [...s.stagePercents] as [number, number, number];
          newPercents[stage - 1] = Math.max(0, percent);
          return { stagePercents: newPercents };
        }),
...
export function migrateDemoAccountState(
  ...
  if (version < 3) {
    ...
    delete s.baseStake;
    if (!s.stagePercents) s.stagePercents = DEFAULT_STAGE_PERCENTS;
  }
  if (version < 4) {
    if (s.martingale) {
  ...
  return {
    ...
    stagePercents: s.stagePercents ?? DEFAULT_STAGE_PERCENTS,
