...
function makeTrade(
  ...
  return {
    ...
    candleTime,
    stage,
    stakeConfigAtOpen: { stage0Amount: 10, stagePercents: [250, 500, 1000] as [number, number, number] },
  };
}
...
describe('useDemoAccountStore — getStageStake', () => {
  it('stage 0 returns stage0Amount literally', () => {
    expect(getStageStake(0, 10, [250, 500, 1000])).toBe(10);
  });

  it('stages 1-3 are round2(stage0Amount * percent / 100)', () => {
    expect(getStageStake(1, 10, [250, 500, 1000])).toBe(25);
    expect(getStageStake(2, 10, [250, 500, 1000])).toBe(50);
    expect(getStageStake(3, 10, [250, 500, 1000])).toBe(100);
  });
...
describe('useDemoAccountStore — setters and reset', () => {
  ...
  it('setStagePercent updates specific percent, does not touch martingale', () => {
    useDemoAccountStore.setState({
      martingale: { 'EURUSD:5m': { stage: 1, halted: false } },
    });

    useDemoAccountStore.getState().setStagePercent(2, 750);

    const state = useDemoAccountStore.getState();
    expect(state.stagePercents).toEqual([250, 750, 1000]);
  ...
  it('resetAccount clears martingale but preserves stage0Amount/stagePercents', () => {
    useDemoAccountStore.setState({
      balance: 500,
      martingale: {
    ...
    expect(state.stagePercents).toEqual([300, 600, 1200]);
...
describe('useDemoAccountStore — persist migration', () => {
  it('migrates from v1 (consecutiveLosses/currentStake) to v4', () => {
    const v1State = {
      balance: 500,
      baseStake: 10,
      consecutiveLosses: 2,
    ...
    expect(result.stagePercents).toEqual([250, 500, 1000]);
  ...
  it('migrates from v3 (baseStake → stage0Amount, halted defaults)', () => {
    const v3State = {
      balance: 500,
      baseStake: 15,
      stagePercents: [250, 500, 1000],
